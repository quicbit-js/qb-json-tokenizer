...
class JsonTokenizer {
    ...
    constructor () {
        ...
        this.ps = jnext.new_ps(
            null,
            { buf2str: (src, off, lim) => { return Buffer.from(src.slice(off, lim).toString()) } },
        )
    }
    nextchunk (chunk) {
        ...
        this.nchunks++
        this.ps.next_src = chunk
        jalign(this.ps, {new_buf: (len) => Buffer.from(new Uint8Array(len))})
    }
    next () {
        if (this.nchunks === 0) {
            throw Error('Before calling next(), JParser expects nextchunk() to be called')
        }
        let ps = this.ps
        if (ps.tok !== 0) {
